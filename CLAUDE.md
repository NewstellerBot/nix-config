...
- `home/` — home-manager modules. Per-host entry points `home/personal.nix` / `home/work.nix` are thin wrappers that import `home/default.nix` (which imports the rest: `shell.nix`, `git.nix`, `tmux.nix`, `direnv.nix`, `claude.nix`, `codex.nix`, `ghostty.nix`) and set `my.claude.*` options — personal gets no MCP servers and only LSP plugins; work adds slack/linear plugins and figma/playwright MCP servers.
